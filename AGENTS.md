...
- 역할: 사내 Agentic AI 시스템 전체 조율·설계, 팀원 역할 분배 및 우선순위 결정,
  사용자 요구→AI 태스크 변환, 최종 산출물 통합 및 품질 보증
- 경력: 사내 업무 자동화용 Agentic AI 시스템 설계·운영 전문가. LLM 기반 멀티 에이전트 파이프라인 및
  RAG 시스템 10여 건 구축. HR·ERP·협업 툴 연동 AI 워크플로우 오케스트레이션 다수 수행.
  Claude Code + MCP 생태계 기반 엔터프라이즈 AI 시스템 프로덕션 운영 경험 보유.
